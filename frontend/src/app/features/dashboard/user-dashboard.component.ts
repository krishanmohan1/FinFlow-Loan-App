...
export class UserDashboardComponent implements OnInit {
  ...
  protected readonly loanTypeOptions: SelectOption[] = [
    { value: 'HOME', label: 'Home loan', hint: 'Property purchase, construction, or home improvement' },
    { value: 'PERSONAL', label: 'Personal loan', hint: 'Medical, family, or planned lifestyle expenses' },
    { value: 'EDUCATION', label: 'Education loan', hint: 'Tuition, accommodation, and study support' },
    { value: 'BUSINESS', label: 'Business loan', hint: 'Working capital, expansion, or equipment' },
    { value: 'AUTO', label: 'Auto loan', hint: 'Car, bike, or commercial vehicle financing' },
    { value: 'GOLD', label: 'Gold loan', hint: 'Short-term secured credit against gold assets' }
  ];
  ...
  protected readonly currentStage = computed(() => {
    ...
    const hasApprovedLoan = this.loans().some((loan) => loan.status === 'APPROVED');
    if (hasApprovedLoan) {
      return 4;
    }
    return 3;
  ...
  protected readonly flowSteps = [
    { id: 1, title: 'Account created', detail: 'You can now work inside your private FinFlow workspace.' },
    { id: 2, title: 'Loan submitted', detail: 'Your application is now moving through the lending pipeline.' },
    { id: 3, title: 'Documents under review', detail: 'Upload supporting files so the admin team can verify them.' },
    { id: 4, title: 'Decision available', detail: 'Track approved, rejected, or under-review outcomes with remarks.' }
  ] as const;
  ...
  protected readonly profileFields = computed(() => {
    ...
    return [
      { label: 'Username', value: profile?.username || this.username() || 'Not available' },
      { label: 'Role', value: profile?.role === 'ADMIN' ? 'Admin' : 'Applicant' },
      { label: 'Profile status', value: this.loans().length > 0 ? 'Active borrower' : 'Ready to apply' }
    ];
  });
  ...
  protected readonly selectedLoanTypeHint = computed(
    () => this.loanTypeOptions.find((option) => option.value === this.loanForm.controls.loanType.value)?.hint ?? ''
  );
  ...
  protected readonly loanForm = this.fb.nonNullable.group({
    ...
    purpose: ['Buying my first home near work and family support.', [Validators.required, Validators.minLength(10), Validators.maxLength(255)]]
  });
  ...
  loadWorkspace(): void {
    ...
    forkJoin({
      ...
      .subscribe({
        ...
        error: (error) => this.error.set(resolveApiError(error, 'Could not load your workspace.'))
      });
  }
  ...
  submitLoan(): void {
    ...
    this.loansApi
      ...
      error: (error) => this.error.set(resolveApiError(error, 'Loan submission failed.'))
    });
  }
  ...
  selectLoanForDocument(loan: LoanApplication): void {
    ...
    this.message.set(`Loan #${loan.id} selected for your next document upload.`);
    this.error.set('');
  }
  ...
  uploadDocument(): void {
    ...
    this.documentsApi
      ...
      error: (error) => this.error.set(resolveApiError(error, 'Document upload failed.'))
    });
  }
  ...
  saveProfile(): void {
    if (this.profileForm.invalid) {
      ...
      this.error.set('Please correct the borrower profile fields before saving.');
      return;
    }
    ...
    this.auth.updateCurrentProfile(this.profileForm.getRawValue() as ProfileUpdateRequest)
      .pipe(finalize(() => this.isSavingProfile.set(false)))
      .subscribe({
        next: (profile) => {
          this.profile.set(profile);
          this.message.set('Your borrower profile has been updated successfully.');
        },
        error: (error) => this.error.set(resolveApiError(error, 'Profile update failed.'))
      });
  }
  ...
  withdrawLoan(loan: LoanApplication): void {
    ...
    this.loansApi.withdraw(loan.id).subscribe({
      ...
      error: (error) => this.error.set(resolveApiError(error, 'Loan withdrawal failed.'))
    });
  }
}
